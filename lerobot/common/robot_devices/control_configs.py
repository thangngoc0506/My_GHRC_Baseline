...
class TeleoperateControlConfig(ControlConfig):
    ...
    # Task selection for walker_s2_sim (task1/task2/task3/task4)
    task: str = "task3"
...
class RecordControlConfig(ControlConfig):
    ...
    def __post_init__(self):
        ...
        if policy_path:
            ...
            # Handle case where config.json doesn't have 'type' key 
            # by adding the 'type' key based on policy type inference
            config_file = os.path.join(policy_path, "config.json")
            if os.path.exists(config_file):
                with open(config_file, "r") as f:
                    config_data = json.load(f)
                if "type" not in config_data:
                    # Infer policy type from path or default to "act"
                    # Check CLI args first for policy.type override
                    policy_type_arg = None
                    for arg in cli_overrides:
                        if arg.startswith("type="):
                            policy_type_arg = arg.split("=")[1]
                            break
                    if policy_type_arg is None:
                        ...
                        if "act" in path_lower:
                            ...
                            policy_type = "act"  # default to act
                    else:
                        policy_type = policy_type_arg
                    # Add 'type' key for draccus parsing
                    config_data["type"] = policy_type
                    # Write modified config to a temp file for draccus parsing
                    temp_config = os.path.join(policy_path, "config_with_type.json")
                    with open(temp_config, "w") as f:
                        json.dump(config_data, f, indent=4)
                    # Use the temp config file path directly
                    self.policy = draccus.parse(PreTrainedConfig, temp_config, args=cli_overrides)
                    # Clean up temp file
